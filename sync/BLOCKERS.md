# Blockers вЂ” Hermes Hub

Date: 2026-06-20

| # | Blocker | Severity | Status |
|---|---------|----------|--------|
| 1 | SSH РґР»СЏ Phase 2 РЅРµРґРѕСЃС‚СѓРїРµРЅ | MEDIUM | Open |
| 2 | Phase 2 РЅРµ РЅР°С‡Р°С‚ | LOW | Requires decision |
| 3 | Production РЅРµ РІРєР»СЋС‡С‘РЅ | LOW | Requires RED gate |

## Resolved

| Blocker | When |
|---------|------|
| Server SSH inventory | вњ… 2026-06-16 |
| Server gates 1-9 | вњ… 2026-06-18 |
| Controlled start | вњ… 2026-06-20 |
...
The first live Russian-header patch for `Файл Малярки` was rolled back.

Facts:

- Local test passed.
- Server-side dummy export created `.xlsx`.
- Telegram sanity after patch: file did not download.
- Rollback restored `/opt/malyarka-telegram-bot/malyarka_core/exports/malyarka_file.py` from:

```text
/opt/malyarka-telegram-bot/malyarka_core/exports/malyarka_file.py.20260620_075550.before_russian_headers
```

Current runtime after rollback:
...
Next investigation should stay read-only first and focus on why Telegram callback/file delivery fails with the Russian-label export while direct export succeeds.
...
- `Скачать Файл Малярки` uses `download_malyarka_file`.
- Handler: `handle_malyarka_file_callback(...)`.
- Preparation: `prepare_malyarka_file_for_user(...)`.
- Export: `export_malyarka_file_xlsx(...)`.
- Telegram send: `message.answer_document(...)`.

Most likely cause of the failed sanity after Russian-header patch:

```text
controlled service restart clears _RUNTIME_COREL_ROWS
```

The callback depends on in-memory `_RUNTIME_COREL_ROWS`. If the user presses an old pre-restart button, rows are gone and the file is not prepared. The direct dummy export can still succeed because it does not use the live in-memory callback state.
...
The next attempt must require a fresh post-restart order preview before pressing `Скачать Файл Малярки`.
...
The old-button hypothesis is closed. The file downloads from a new preview, but English technical fields remain:
...
- Actual Telegram button path is `download_malyarka_file` → `handle_malyarka_file_callback(...)` → `prepare_malyarka_file_for_user(...)` → `malyarka_core.exports.malyarka_file.export_malyarka_file_xlsx(...)`.
- English fields are currently in live `/opt/malyarka-telegram-bot/malyarka_core/exports/malyarka_file.py`.
- No second export path for the button was found.
- The fresh retest showed English headers because the previous Russian patch had already been rolled back.
...
## 2026-06-20 — Malyarka File Russian export blocker resolved
...
The blocker `MALYARKA_FILE_EXPORT_USES_ENGLISH_HEADERS_AND_STATUSES` is resolved by the controlled reapply patch and fresh Telegram test.

Current state:

- File downloads.
- Headers/statuses are Russian.
