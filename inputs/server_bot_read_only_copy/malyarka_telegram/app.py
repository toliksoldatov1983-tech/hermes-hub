...
async def _run_aiogram_polling(config: TelegramConfig) -> None:
    ...
    from malyarka_telegram.keyboards import CopyKeyboardSpec, MAIN_BUTTONS
    ...
    main_reply_markup = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=label)] for label in MAIN_BUTTONS],
        resize_keyboard=True,
        input_field_placeholder="Выберите режим или напишите текст",
    )
    ...
    async def _handle_message(message: Any) -> None:
        ...
        if text:
            ...
            await message.answer(handle_photo_message(), reply_markup=main_reply_markup)
