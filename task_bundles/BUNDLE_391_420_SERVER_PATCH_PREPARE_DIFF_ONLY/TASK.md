...
Подготовить план diff без применения. Не читать secrets и не менять сервер.

## ОЖИДАЕМЫЙ РЕЗУЛЬТАТ

Создан prepare diff plan. Patch не применён.

## Conveyor rule

Остановиться перед выполнением. Вернуть review report and ask user/ChatGPT.
...
`REPORT_BUNDLE_391_420_SERVER_PATCH_PREPARE_DIFF_ONLY.md`

## Status codes

Use one of:

- `SERVER_PATCH_PREPARE_DIFF_ONLY_COMPLETE`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_INCOMPLETE`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_BLOCKED_MISSING_PREREQUISITES`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_STOPPED_REVIEW_REQUIRED`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_STOPPED_APPROVAL_REQUIRED`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_STOPPED_SAFETY_RISK`
- `SERVER_PATCH_PREPARE_DIFF_ONLY_BLOCKED_NOT_READY`
